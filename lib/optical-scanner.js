...
export class OpticalScanner {
  ...
  *  - Performance: 30-60 scans per second (matches video frame rate)
  ...
  *   _scanContinuousFrameCallback() - 150x faster >
  *   else >
  ...
  *
  * PERFORMANCE IMPACT:
  * - Original (bedrock-vue-barcode-scanner): 30-60 fps frame-accurate.
  * - Iteration 1 (pre-refactor): 0.4 fps polling-only (regression).
  * - Current (post-refactor): 30-60 fps for video, 0.4 fps fallback.
  *
  * HISTORICAL CONTEXT:
  ...
  */
  async scanContinuous(video, options = {}) {
    ...
    if(video instanceof HTMLVideoElement) {
      ...
      console.log('Frame-callback path: Using requestVideoFrameCallback()' +
    ...
    console.warn(
      ...
      '  Recommendation: Use HTMLVideoElement for 150x better performance ' +
      '(30-60 fps)'
    );
    console.log('Polling fallback: Source is not video element, using' +
      'setTimeout() approach');
  ...
  */
  async _scanContinuousFrameCallback(video, options) {
    ...
    console.log('Frame-callback scan started:', {
      formats: formats.join(', '),
      timeout: timeoutMs === 0 ? 'No timeout' : `${timeoutMs}ms`,
      hasSignal: !!signal
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      const scanFrame = async () => {
        try {
          ...
          // Only log unexpected errors (silence "no results")
