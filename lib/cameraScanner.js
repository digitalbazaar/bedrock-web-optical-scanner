...
import {getDynamsoftLicense} from './config.js';
import {OpticalScanner} from './opticalScanner.js';

const logger = createLogger('[CameraScanner]');
...
export class CameraScanner extends EventEmitter {
  constructor(options = {}) {
    ...
    const {
      // scanType = 'mrz' | 'barcode' | 'auto'
      scanType = options.scanType || 'barcode',
      mrzMode = options.mrzMode || (scanType === 'mrz' ? 'camera' : 'element'),
      licenseKey = getDynamsoftLicense(),
      // scanMode = 'first' | 'all' | 'exhaustive'
      scanMode = options.scanMode || 'first',
    ...
    if(formats !== null) {
      const validFormats = ['qr_code', 'pdf417', 'pdf417_enhanced', 'mrz'];
      const invalidFormats =
        formats.filter(format => !validFormats.includes(format));
    ...
    // Store container for plugin options
    ...
    isDebugEnabled() && console.log(
      ...logger.prefix('CameraScanner initialized with config:', this.config));
  }
  ...
  *
  * If explicit formats provided via constructor, use those.
  * Otherwise, use scanType defaults.
  *
  * @private
  ...
  */
  _getFormats(scanType) {
    ...
    switch(scanType) {
      ...
      // Open-source defaults 'pdf417', 'qr_code'
      case 'barcode':
        return ['pdf417', 'qr_code'];

      case 'auto':
        return ['qr_code', 'pdf417', 'mrz'];
  ...
  */
  _buildPluginOptions(formats, licenseKey, container, mrzMode) {
    ...
    if(!licenseKey) {
      isDebugEnabled() && console.log(
        ...logger.prefix('No license key provided'));
    ...
    formats.forEach(format => {
      switch(format) {
        ...
        default:
          console.info(
            `No special plugin options needed for format: ${format}`);
      }
    });
    ...
    switch(mrzMode) {
      case 'camera':
        ...
        return {
          ...
          mrzMode,
          scannerConfig: {
            // Critical for Dynamsoft native UI
            container,
            scannerViewConfig,
            resultViewConfig
          }
  ...
  */
  _determineScanningMode(scanType) {
    ...
    // even for formats=['pdf417_enhanced] override param
    if(scanType === 'barcode') {
      return true;
    }
  ...
  */
  _getPluginsForScanType() {
    ...
    // Get the actual formats that will be used
    const formats = this._getFormats(this.config.scanType);

    // Map formats to plugins
    const pluginMap = {
    ...
    // Only register plugins for requested formats
  ...
  */
  async start(container, options = {}) {
    ...
    if(isDebugEnabled()) {
      console.log(...logger.prefix('=== CAMERA SCANNER START ==='));
      console.log(...logger.prefix('Scan type:', this.config.scanType));
      console.log(...logger.prefix('MRZ mode:', this.config.mrzMode));
      console.log(...logger.prefix('Container provided:', !!container));
      console.log(...logger.prefix('Auto-scan requested:', autoScan));
    }
    ...
    if(this._stream && this._container) {
      isDebugEnabled() && console.log(...logger.prefix(
        'Camera already started, returning existing setup'));
      return {
        success: true,
    ...
    try {
      ...
      // isDebugEnabled() && console.log(
      //   ...logger.prefix('Container stored for plugin configuration'));

      const formats = this._getFormats(this.config.scanType);
      // isDebugEnabled() && console.log(
      //   ...logger.prefix('Formats determined:', formats));

      // ===== CONTAINER SETUP =====
      if(this.config.scanType === 'mrz' && this.config.mrzMode === 'camera') {
        console.info('MRZ camera mode - Dynamsoft will manage container');
      } else {
        isDebugEnabled() && console.log(
          ...logger.prefix(`Setting up video element for` +
      ...
      // isDebugEnabled() && console.log(
      //   ...logger.prefix('Plugin options built with container reference'));

      // ===== AUTO-START LOGIC =====
      let autoScanStarted = false;
      if(autoScan) {
        console.info('Auto-starting scanning based on scanType:',
          this.config.scanType);

        // Start scanning in the background (non-blocking)
        setTimeout(() => {
          this.scan(options).catch(error => {
            console.error('Auto-scan completed or failed:', error.message);
      ...
      const result = {
        ...
        mrzMode: this.config.mrzMode,
        formats,
        // indicates if auto-scan was initiated
        autoScanStarted
      };
  ...
  */
  async startScanning(options = {}) {
    ...
    isDebugEnabled() && console.log(
      ...logger.prefix('Manual scanning initiated'));
  ...
  */
  async _setupVideoElement(container) {
    ...
    console.info('Video element created and inserted into container');
  }
  ...
  */
  async scan(options = {}) {
    if(isDebugEnabled()) {
      console.log(...logger.prefix('=== CAMERA SCANNER SCAN ENTRY ==='));
      console.log(...logger.prefix('Scan type:', this.config.scanType));
      console.log(...logger.prefix('MRZ mode:', this.config.mrzMode));
    }
    ...
    try {
      ...
      if(formats.includes('mrz') && !this.config.licenseKey) {
        throw new Error(
          'MRZ scanning requires a Dynamsoft license key. ' +
          'Please configure it in your bedrock config.'
        );
      }
      ...
      isDebugEnabled() && console.log(
        ...logger.prefix('CameraScanner.scan() config:', {
          formats,
          timeoutMs,
          useContinuous,
      ...
      if(useContinuous) {
        // BARCODE qr_code & pdf417 > Continuous scanning
        console.info('Delegating to OpticalScanner.scanContinuous()');
        ...
        // MRZ > Single scan
        console.info('Delegating to OpticalScanner.scan()');
  ...
  */
  async scanAny(options = {}) {
    isDebugEnabled() && console.log(
      ...logger.prefix('=== CAMERA SCANNER SCAN ANY (AUTO-DETECT) ==='));
    ...
    try {
      ...
      if(allFormats.includes('mrz') && !this.config.licenseKey) {
        throw new Error(
          'MRZ scanning requires a Dynamsoft license key. ' +
          'Please configure it in your bedrock config.'
        );
      ...
      isDebugEnabled() && console.log(
        ...logger.prefix('Auto-detect configuration:', {
          allFormats,
      ...
      // ===== DIRECT DELEGATION TO OPTICAL SCANNER =====
      isDebugEnabled() && console.log(
        ...logger.prefix('Delegating to OpticalScanner.scanAny()'));
      ...
      if(results && results.length > 0) {
        isDebugEnabled() && console.log(
          ...logger.prefix('CameraScanner: Auto-stopping camera after ' +
          'successful'));
        this.stop();
      }
  ...
  */
  ...
  _formatScanResults(results) {
    ...
    isDebugEnabled() && console.log(
      ...logger.prefix('Formatting scan result:', result.format));
  ...
  */
  ...
  stop() {
    ...
    this._container = null;
    console.info('CameraScanner stopped and cleaned up');
  }
  ...
  */
  async setTorch(enabled) {
    ...
    try {
      isDebugEnabled() && console.log(
        ...logger.prefix(`Setting torch: ${currentState} -> ${targetState}`));
      ...
      isDebugEnabled() && console.log(...logger.prefix(
        `Torch ${targetState ? 'enabled' : 'disabled'} successfully`));
      return targetState;
  ...
  */
  async setZoom(level) {
    ...
    try {
      ...
      isDebugEnabled() && console.log(
        ...logger.prefix(`Setting zoom: ${currentLevel} -> ${level}`));
      ...
      isDebugEnabled() && console.log(
        ...logger.prefix(`Zoom set to ${level} successfully`));
      return level;
  ...
  */
  async switchCamera(deviceId) {
    ...
    try {
      isDebugEnabled() && console.log(...logger.prefix(
        `Switching camera to: ${targetCamera.label || deviceId}`));
      ...
      isDebugEnabled() && console.log(
        ...logger.prefix('Starting new camera with constraints:', constraints));
      ...
      isDebugEnabled() && console.log(...logger.prefix(
        `Successfully switched to camera: ${targetCamera.label || deviceId}`
      ));
      return this._videoElement;
      ...
      try {
        isDebugEnabled() && console.log(
          ...logger.prefix('Attempting to recover original camera...'));
        // Restart with default camera
        await this.start(this._container);
      } catch(recoveryError) {
  ...
  async scanFile(files) {
    ...
    try {
      ...
      const filePluginOptions = this._buildPluginOptions(
        formats,
        this.config.licenseKey,
        // No container needed for file scanning
        null,
        // Force file mode for MRZ
        'file'
      );
