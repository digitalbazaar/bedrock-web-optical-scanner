...
export const mrzPlugin = {
  ...
  async _scanFromCamera(licenseKey, scannerConfig, signal) {
    ...
    // ADD THIS: Check for DOM elements after a delay
    setTimeout(() => {
      // eslint-disable-next-line max-len
      const dynamSoftElements = document.querySelectorAll('[class*="dynamsoft"], [id*="dynamsoft"], [class*="dce"], [id*="dce"], [class*="mrz"], [id*="mrz"]');
      console.log('📷 Dynamsoft DOM elements found:', dynamSoftElements.length);
      dynamSoftElements.forEach(el => {
        console.log('📷 Element:',
          el.tagName, el.className, el.style.display, el.style.visibility);
      });
    }, 2000);
