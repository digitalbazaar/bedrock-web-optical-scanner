...
const logger = createLogger('[MRZ Plugin]');
...
export const mrzPlugin = {
  ...
  */
  async scan(source, options = {}) {
    if(isDebugEnabled()) {
      console.log(
        ...logger.prefix('=== MRZ PLUGIN DEBUG ==='));
      // console.log(
      //   ...logger.prefix('Source type:', source?.constructor?.name));
      // console.log(...logger.prefix('Options received:', options));
      // console.log(...logger.prefix(
      //   'License key:', options.licenseKey ? 'PROVIDED' : 'MISSING'));
      // console.log(...logger.prefix('MRZ Mode:', options.mrzMode));
      console.log(...logger.prefix('Scanner Config:', options.scannerConfig));
      console.log(...logger.prefix('Target Container:',
    ...
    if(isDebugEnabled()) {
      // console.log(
      //   ...logger.prefix('MRZ Plugin: About to enter switch statement'));
      // console.log(...logger.prefix('MRZ Mode resolved to:', mrzMode));
    }
    ...
    try {
      ...
      switch(mrzMode) {
        case 'camera':
          isDebugEnabled() &&
            console.log(...logger.prefix('MRZ Plugin: Entering camera mode'));
          result = await this._scanFromCamera(
            licenseKey, scannerConfig, signal
          );
          isDebugEnabled() && console.log(
            ...logger.prefix('MRZ Plugin: Camera scan completed:', result));
          break;
  ...
  */
  async _scanFromCamera(licenseKey, scannerConfig, signal) {
    isDebugEnabled() &&
      console.log(...logger.prefix('_scanFromCamera called with:',
        {
          licenseKey: !!licenseKey,
          scannerConfig, targetContainer: !!scannerConfig.targetContainer
        }));

    const {container: targetContainer, ...dynamSoftConfig} = scannerConfig;
    if(isDebugEnabled()) {
      console.log(
        ...logger.prefix('_scanFromCamera: targetContainer >> ', targetContainer
        ));
      console.log(
        ...logger.prefix('_scanFromCamera: dynamSoftConfig >> ', dynamSoftConfig
        ));
    }
    ...
    const mrzScanner = new MRZScanner({
      ...
      showResultView: false, //TODO move this to _buildMrzPluginOptions() later
      ...dynamSoftConfig,
      resultViewConfig: {
        ...dynamSoftConfig.resultViewConfig,
        onDone: result => {
          // Result will be handled in the promise resolution
          console.info('Dynamsoft onDone callback triggered:', result);
          return result;
        }
      }
    });

    console.info('MRZScanner created, about to call launch()');
    ...
    try {
      ...
      isDebugEnabled() && console.log(
        ...logger.prefix('MRZScanner.launch() completed:', result));
  ...
  */
  async _scanFromFile(file, licenseKey, scannerConfig, signal) {
    if(isDebugEnabled()) {
      // console.log(...logger.prefix('=== _scanFromFile ENTRY ==='));
      // console.log(...logger.prefix('File:', file?.name, file?.size));
      // console.log(...logger.prefix('License key provided:', !!licenseKey));
      // console.log(...logger.prefix('Creating MRZScanner...'));
    }
    ...
    console.info('_scanFromFile: MRZScanner created successfully.');

    try {
      // Check for abort before scanning
      signal?.throwIfAborted();
      isDebugEnabled() && console.log(
        ...logger.prefix('About to call mrzScanner.launch(file)...'));

      const result = await mrzScanner.launch(file);
      isDebugEnabled() && console.log(
        ...logger.prefix('_scanFromFile: MRZ scan result received:', !!result));
      ...
      console.info('Cleaning up MRZ scanner...');
      // Clean up resources
      if(mrzScanner.destroy) {
        mrzScanner.destroy();
      }
  ...
  */
  async _scanFromElement(source, licenseKey, scannerConfig, signal) {
    ...
    try {
      ...
      // DEBUG LOG
      if(isDebugEnabled()) {
        console.log(...logger.prefix('=== ABOUT TO CALL _scanFromFile ==='));
        // console.log(
        //   ...logger.prefix('File created:', !!file, file?.name, file?.size));
        // console.log(...logger.prefix(
        //   '_scanFromFile method exists:', typeof this._scanFromFile));
      }
  ...
  */
  async _elementToFile(source) {
    if(isDebugEnabled()) {
      console.log(...logger.prefix('=== ELEMENT TO FILE DEBUG ==='));
      // console.log(...logger.prefix('Source type:', source.constructor.name));
      // console.log(...logger.prefix(
      //   'Is HTMLVideoElement:', source instanceof HTMLVideoElement));
    }
    ...
    return new Promise((resolve, reject) => {
      if(isDebugEnabled()) {
        console.log(...logger.prefix('=== CANVAS STATE DEBUG ==='));
        // console.log(...logger.prefix(
        //   'Canvas width/height:', canvas.width, canvas.height));
        // console.log(...logger.prefix(
        //   'Canvas context exists:', !!canvas.getContext('2d')));
      }
      ...
      try {
        ...
        isDebugEnabled() && console.log(...logger.prefix(
          'Canvas has pixel data:', imageData.data[0] !== undefined));
      } catch(e) {
        console.error('Canvas pixel read failed:', e.message);
      }
